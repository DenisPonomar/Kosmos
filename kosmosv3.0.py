...
def Planets():
    ...
    x = 2*width * random.random()
    g = int(255 * random.random())
    random.seed(int(mir_y))
    y = 2*height * random.random()
    ...
    if y > 2*height:
        planets_y = planets_y - 2*height
    pygame.draw.circle(screen, color, [int(x), int(y)], 50, 0)
def Back():
