...
if len(sys.argv) == 1:
    ...
    class MyWindowClass(QtGui.QMainWindow, QtGui.QWidget, form_class):
        ...
        def button_clicked0(self):
            ...
            if Game_Name not in Vse_Game_Name[0]:
                ...
                Vse_Game_Name[1].append('10000') #количество топлива
                Vse_Game_Name[2].append('0') #количество товара
                Vse_Game_Name[3].append('1000') #количество денег
                Vse_Game_Name[4].append('0') #игрок не в бесконечной Вселенной
                Vse_Game_Name[5].append('633') #координаты икс игрока
                Vse_Game_Name[6].append('300') #координаты игрек игрока
                Vse_Game_Name[7].append('0') #координаты мир_икс игрока
                Vse_Game_Name[8].append('0') #координаты мир_игрек игрока
                Vse_Game_Name[9].append('0') #значение a в бесконечной Вселенной
                Vse_Game_Name[10].append('0') #значение b в бесконечной Вселенной
                Vse_Game_Name[11].append('633') #координаты икс игрока в бесконечной Вселенной
                Vse_Game_Name[12].append('300') #координаты игрек игрока в бесконечной Вселенной
    ...
    if "Game_Name" in locals():
        ...
        if int(Vse_Game_Name[4][index1])%2 == 1:
            f = open('game_name_vselennaya.txt', 'w')
            f.write(str(Game_Name))
            f.close()
            os.system("vselennaja.py")
            sys.exit()
if len(sys.argv) == 2:
    Vse_Game_Name = []
    f = open('Vse_Game_Name.txt', 'r')
...
def Torgovlya():
    ...
    if tsena_tovar == 42 or tsena_toplivo == 42:
        keystate = pygame.key.get_pressed()
        if keystate[pygame.K_KP5]:
            ...
            index1 =  Vse_Game_Name[0].index(str(Game_Name)) 
            Vse_Game_Name[4][index1] = str(int(Vse_Game_Name[4][index1])+1)
            f = open('Vse_Game_Name.txt', 'w')
            f.write(str(Vse_Game_Name) + '\n')
            f.close()
            global running
...
def razg():
    x_razg = 600
    global razgovor
    if int(mir_x) == 42 or int(mir_y) == 42:
        chislo = "Zdravstvuj, putnik! Ja znaju, zachem ty prishyol."
        score_surf = score_font.render(chislo, 1, (223, 223, 223))
        screen.blit(score_surf, [x_razg, 0])
        if razg_time > 1.5:
            chislo = "No ja znaju ne bolshe, chem drugie torgovtsy." 
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 30])
        if razg_time > 3:
            chislo = "Otvet lezhit na poverhnosti, no nemnogo glubzhe."
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 60])
        if razg_time > 4.5:
            chislo = "Poprobuj poiskat. V nashej Vselennol beskonechost"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 90])
        if razg_time > 6:
            chislo = "mirov, i gde-to ty obyazftelno najdyosh eto chislo"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 120])
        if razg_time > 12:
            razgovor = 0
    else:
        chislo = "Zdravstvuj, putnik! Rasskazat tebe legendu?"
        score_surf = score_font.render(chislo, 1, (223, 223, 223))
        screen.blit(score_surf, [x_razg, 0])
        if razg_time > 1.5:
            chislo = "Nachalos eto tak davno, chto nikto uzhe i" 
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 30])
        if razg_time > 3:
            chislo = "ne pomnit, kak davno eto bylo. No v te"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 60])
        if razg_time > 4.5:
            chislo = "vremena nasha Vselennaya byla beskonechnoj,"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 90])
        if razg_time > 6:
            chislo = "a ne mnozhestvo zamknutyh, kak sejchas."
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 120])
        if razg_time > 7.5:
            chislo = "No posle vselenskoj katastrofy imeem,"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 150])
        if razg_time > 9:
            chislo = "chto imeem. Vprochem govoryat, chto v mestah,"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 180])
        if razg_time > 10.5:
            chislo = "gde ty vstretish chislo 42, tam i"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
            screen.blit(score_surf, [x_razg, 210])
        if razg_time > 12:
            chislo = "naydyosh vhod v beskonechnuyu Vselennuyu!"
            score_surf = score_font.render(chislo, 1, (223, 223, 223))
...
if raz == 2:
    try:
        ...
        os.system("echo V itoge ja imeju rovnym schyotom nichego.\
Po factu ja nashyol cho hotel, no eto okazalos ne bolee \
chem illyuziej. A nastoyatshaja Vselennaja kak byla daleka,\
tak takoj zhe i ostalos. Hotya ja osobo ne hotel tuda popast,\
no vsyo ravno kak-to bolno. Slovno tebe dali konfetku, a \
vnutri nejo okazalsya myakish ot hleba. I delat bolshe \
nechego, i podskazok nikto nikakih ne dayot. Pozhaluj budu \
dalshe zanimatsya torgovlej - hot v etom est kakoj-to smysl. > txt.txt")
        os.system("notepad txt.txt")
loc.append(float(Vse_Game_Name[5][index1]))
loc.append(float(Vse_Game_Name[6][index1]))
mir_x = float(Vse_Game_Name[7][index1])
mir_y = float(Vse_Game_Name[8][index1])
...
Vse_Game_Name[5][index] = str(loc[0])
Vse_Game_Name[6][index] = str(loc[1])
Vse_Game_Name[7][index1] = str(mir_x)
Vse_Game_Name[8][index1] = str(mir_y)
